...
<head>
    ...
    <meta name="description" content="Fotoverso - Busqueda Filtrada de Mis Fotos">
    <meta name="keywords" content="HTML, CSS, PHP, Fotos, Fotoverso, Imágenes, Red-Social, Comunicación">
    <title>Fotoverso - Busqueda Filtrada de Mis Fotos</title>
...
<body>
    ...
    <div class="rectangle">
        <div class="left-section">
            <p class="info" style="text-shadow: 4px 3px 0px rgba(171, 183, 183, 1), 9px 8px 0px rgba(0,0,0,0.15); font-size: xxx-large"> BUSQUEDA MIS FOTOS </p>
            <a href="../php/Sección_MisFotos.php" style="margin-top: 5px; font-size: 24px; background: #9ab7b7" class="btn btn-outline-dark"> Retornar </a>
        </div>
        ...
        <div class="right-container">
            ...
            <?php
                ...
                if (!empty($SEARCH_ORDER)) {
                    if ($SEARCH_ORDER == "Ascendente") {
                        if (!empty($SEARCH_USER) && !empty($SEARCH_CATEGORY)) {
                            $object = mysqli_query($conn, "CALL FOTOS_ASCconUSERyCATEGORY('$owner', '$CATEGORY_ID')");
                        } else if (!empty($SEARCH_USER) && empty($SEARCH_CATEGORY)) {
                            $object = mysqli_query($conn, "CALL FOTOS_ASCconUSER('$owner')");
                        } else if (empty($SEARCH_USER) && !empty($SEARCH_CATEGORY)) {
                        ...
                        if (!empty($SEARCH_USER) && !empty($SEARCH_CATEGORY)) {
                            $object = mysqli_query($conn, "CALL FOTOS_DESconUSERyCATEGORY('$owner', '$CATEGORY_ID')");
                        } else if (!empty($SEARCH_USER) && empty($SEARCH_CATEGORY)) {
                            $object = mysqli_query($conn, "CALL FOTOS_DESconUSER('$owner')");
                        } else if (empty($SEARCH_USER) && !empty($SEARCH_CATEGORY)) {
                    ...
                    if (!empty($SEARCH_USER) && !empty($SEARCH_CATEGORY)) {
                        $object = mysqli_query($conn, "CALL FOTOS_USERyCATEGORY('$owner', '$CATEGORY_ID')");
                    } else if (!empty($SEARCH_USER) && empty($SEARCH_CATEGORY)) {
                        $object = mysqli_query($conn, "CALL FOTOS_USER('$owner')");
                    } else if (empty($SEARCH_USER) && !empty($SEARCH_CATEGORY)) {
                ...
                foreach ($TEMP_STORAGE_FOTOS as $fotos) {
                    ...
                    // $object = mysqli_query($conn, "CALL GET_FOTOGRAFO_CON_ID('$TEMP_ID')");
