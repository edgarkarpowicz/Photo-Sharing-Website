...
<body>
    ...
    <div class="rectangle">
        ...
        <p class="info" style="text-shadow: 4px 3px 0px rgba(171, 183, 183, 1), 9px 8px 0px rgba(0,0,0,0.15); font-size: xxx-large"> MIS FOTOS </p>
            <a href="../php/Subir_Foto.php" class="btn btn-outline-dark" style="margin-top: 5px; font-size: 24px; background: #9ab7b7">Subir Foto</a>
            <a href="../php/Mis_Galerias.php" class="btn btn-outline-dark" style="margin-top: 5px; font-size: 24px; background: #9ab7b7">Galerias</a>
            <a href="../php/Filtros_MisFotos.php" class="btn btn-outline-dark" style="margin-top: 5px; font-size: 24px; background: #9ab7b7">Filtros</a>
        </div>
      ...
      <div class="row">
        <div class="right-container">
            <div class="right-section"> 
                ...
                $object = mysqli_query($conn, "CALL LISTAR_FOTOS_OWNER('$Nickname')");
                //while($img_name = mysqli_fetch_array($result)) {
                    //echo $img_name['IMG_Perfil']; // Print a single column data
                    //echo print_r($img_name);       // Print the entire row data
                // } 
                ...
                foreach($TEMP_STORAGE_FOTOS as $fotos) {
                    ...
                    if ($lat != 0 and $lon != 0) {
                        ...
                        <div class='col'>
                            ...
                            <div class='image-info'>
                                <p><u style='text-decoration: underline'>ID Interno:</u> {$fotos['ID']} <br>
                                <u style='text-decoration: underline'>Categoria:</u> {$result[1]} <br> 
                            ...
                            <div style='text-align: center;'>
                                <div>
                                    <form action='../php/Editar_Foto.php' method='POST' style='margin: 5px; margin-top: 0px'>
                                        <input type='hidden' name='internal_id' value='{$fotos['ID']}'>
                                        <input type='submit' class='btn btn-outline-dark' style='background: #9ab7b7' value='EDITAR ESTA FOTO'>
                                    </form>
                                    <form action='../php/Borrar_Foto.php' method='POST' style='margin: 5px'>
                                        <input type='hidden' name='internal_id' value='{$fotos['ID']}'>
                                        <input type='submit' class='btn btn-outline-dark' style='background: #9ab7b7' value='BORRAR ESTA FOTO'>
                                    </form>
                                    <form action='../php/Agregar_Galeria.php' method='POST' style='margin: 5px'>
                                        <input type='hidden' name='internal_id' value='{$fotos['ID']}'>
                                        <input type='submit' class='btn btn-outline-dark' style='background: #9ab7b7' value='AGREGAR A UNA GALERIA'>
                                    </form>
                                    <a href='$mapsUrl' target='_blank' class='btn btn-outline-dark' style='background: #9ab7b7; letter-spacing: 0px'>VER EN MAPA</a>
                                </div>
                            </div>
                        </div>";
                    } 
                    ...
                    <div class='col'>
                        <img class='square-image' src='{$target_file}' alt='Imagen' style='border: 5px rgba(255,255,255,0.7) solid'>
                        <div class='image-info'>
                            <p> <u style='text-decoration: underline'>ID Interno:</u> {$fotos['ID']} <br> <u style='text-decoration: underline'>Categoria:</u> {$result[1]} <br> <u style='text-decoration: underline'>Fecha:</u> {$fotos['Fecha']} <br> <u style='text-decoration: underline'>Descripcion:</u> {$fotos['Descripcion']} </p>
                        </div>
                        <div style='text-align: center;'>
                            <div>
                                <form action='../php/Editar_Foto.php' method='POST' style='margin: 5px; margin-top: 0px'>
